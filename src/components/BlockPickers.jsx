...
export default function BlockPickers({ onSelectBlock, activeDayKey, onSelectDay, activeDayLabel, daysCount = 5 }) {
    ...
    return (
        <section className="bg-zinc-900/40 border border-zinc-800/60 p-3 sm:p-4 rounded-2xl mb-4 sm:mb-6 print:hidden relative space-y-3">
            ...
            {editingItem && (
                <div
                    className="fixed inset-0 z-50 flex items-start pt-8 sm:items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-xs overflow-y-auto transition-opacity duration-300"
                    onClick={(e) => {
                ...
                >
                    <div ref={modalRef}
                        ...
                        className="w-full max-w-sm bg-[#121214] border border-zinc-800 rounded-2xl p-5 shadow-2xl space-y-3.5 max-h-[85vh] sm:max-h-[90vh] overflow-y-auto pb-safe transform transition-all duration-300 ease-out translate-y-0"
                    >
